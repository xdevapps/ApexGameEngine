...
// 			real_t minSep = 0.0;
			...
			for (auto i = 0; i < usedContacts; i++) {
				...
				if (sepVel < minSep && (sepVel < (real_t)0.0 || contacts[i].Penetration > 0)) {
					minSep = sepVel;
					minSepIdx = i;
				}
			}
	...
	uint32_t ParticleRod::AddContact(ParticleContact* contact, uint32_t limit) const
	{
		...
		real_t lenDiff = glm::abs(length - m_Length);
		if (lenDiff < 1e-6) return 0; // No contact added if not extended or contracted
		
		contact->Particles[0] = m_Particles[0];
		contact->Particles[1] = m_Particles[1];
		
		vec3_t normal = m_Particles[1]->Position - m_Particles[0]->Position;
		contact->ContactNormal = glm::normalize(normal) * (real_t)(length > m_Length ? 1.0 : -1.0);
		
		contact->Penetration = lenDiff;
